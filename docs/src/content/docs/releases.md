...
The repository has five independently versioned components:
...
The MCP server is an application-integrated adapter under `lyra_app/mcp/`, not
an independently installable or versioned component. The application container
also embeds the SDK and utilities packages. Changes to those package paths can
therefore release both the changed package and `lyra-app`. Changes limited to
documentation, `lyra-api`, or `lyra-tui` do not release the application image.
...
6. Each independently released Python package tag is installed and imported in
   a fresh uv project.
7. When `lyra-app` is among the released components, the same workflow builds
   the versioned Docker image, verifies the integrated MCP adapter can be
   imported, and updates the `latest` image tag.
